...
group by year, transaction_description
HAVING COUNT(*) > 1
...
from transactions trans
inner join accounts acct on acct.account_id = trans.account_id
group by year, acct.institution,
         acct.account_name,
...
select EXTRACT(YEAR FROM TRANSACTION_DATE) as year,  CASE when trans.merchant_tag is not null then trans.merchant_tag else trans.description END AS transaction_description,
       count(*) as count_transactions,
       sum(trans.amount) as total_transactions
from transactions trans
inner join accounts acct on acct.account_id = trans.account_id
